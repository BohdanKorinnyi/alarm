...
public class LanguageServiceImpl implements LanguageService {
  ...
  public void save(LanguageDto languageDto) {
    ...
    if (nonNull(language)) {
      throw new SaveExistingEntityException(
          "Language " + language.getName() + " already exists in with code " + language.getCode());
    }
  ...
  public void update(LanguageDto languageDto) {
    ...
    try {
      Language language =
          languageRepository.findByCodeOrName(languageDto.getCode(), languageDto.getName());
      if (nonNull(language)) {
        language.setIntro(languageDto.getIntro());
        languageRepository.save(language);
        return;
    ...
    throw new IllegalStateException(
        "There is no language with code "
            + languageDto.getCode()
            + " or name "
            + languageDto.getName());
  }
