...
public class TwilioServiceImpl implements TwilioService {
    ...
    private BiFunction<String, String, Say> alarmMessage = (message, code) -> new Say.Builder(message)
            .voice(Say.Voice.MAN).language(getTwilioLanguage(code)).build();

    @Override
    @SneakyThrows
    public Call makeCall(String toNumber, long clientId) {
        return Call.creator(new PhoneNumber(toNumber), fromPhoneNumber,
    ...
    public String buildSayResponse(String message, Language language) {
        ...
        return new VoiceResponse.Builder()
                .say(intro).pause(PAUSE).say(intro).pause(PAUSE)
    ...
    private Function<Language, Say> intro() {
        return language -> new Say.Builder(language.getIntro())
    ...
    private Say.Language getTwilioLanguage(String code) {
        return Arrays.stream(Say.Language.values())
                ...
                .findFirst()
                .orElseThrow(() -> new UnsupportedLanguageException("Language " + code + " doesn't support"));
    }
}
