...
public class S3ServiceImpl implements S3Service {
  ...
  public List<String> findAllBuckets() {
    return amazonS3.listBuckets().stream().map(Bucket::getName).collect(toList());
  }
  ...
  public String upload(MultipartFile file) {
    try {
      String fileKey =
          UUID.randomUUID().toString() + "." + Files.getFileExtension(file.getOriginalFilename());
      ObjectMetadata metadata = new ObjectMetadata();
      metadata.setContentLength(file.getSize());
      metadata.setContentType(file.getContentType());
      amazonS3.putObject(
          new PutObjectRequest(
                  amazonS3Configuration.getBucket(), fileKey, file.getInputStream(), metadata)
